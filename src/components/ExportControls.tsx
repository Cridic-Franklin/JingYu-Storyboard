...
import { shotSpatialData } from '../lib/spatial';
import { describeScene } from '../lib/scene';
const annotationLabels: Record<keyof CameraAnnotations, MessageKey> = { names: 'names', types: 'types', facing: 'facingArrows', front: 'frontArrows', coordinates: 'coordinates', bounds: 'bounds', measurements: 'measurements' };
...
export function ExportControls({ shot }: { shot: Shot }) {
  const t = useT(); const [target, setTarget] = useState<'camera' | 'plan' | null>(null); const [preset, setPreset] = useState<'clean' | 'directorFrame' | 'aiReference'>('clean');
  ...
  function choosePreset(value: typeof preset) { setPreset(value); setAnnotations({ ...defaultAnnotations(), ...(value === 'directorFrame' ? { names: true, facing: true, front: true, measurements: true } : value === 'aiReference' ? { names: true, facing: true, front: true, coordinates: true } : {}) }); setGuides(value === 'directorFrame' ? [...shot.overlays] : []); }
  async function output(copy = false) {
  ...
  return <><div className="export-actions"><button onClick={() => { setTarget('camera'); choosePreset('clean'); setMessage(''); }}>{t('exportCamera')}</button><button onClick={() => { setTarget('plan'); setMessage(''); }}>{t('exportPlan')}</button><button onClick={() => projectStorage.download(new Blob([JSON.stringify(shotSpatialData(shot), null, 2)], { type: 'application/json' }), `${base}_spatial.json`)}>{t('exportJSON')}</button><button onClick={() => projectStorage.download(new Blob([describeScene(shot)], { type: 'text/plain;charset=utf-8' }), `${base}_spatial.txt`)}>{t('exportText')}</button></div>
