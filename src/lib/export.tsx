...
export function registerCapture(id: string, fn: CameraCapture) { captures.set(id, fn); return () => { if (captures.get(id) === fn) captures.delete(id); }; }
...
export async function cameraPNG(shot: Shot, width: number, annotations: CameraAnnotations, guides: Overlay[]): Promise<Blob> {
  const capture = captures.get(shot.id); if (!capture) throw new Error('Camera preview is not ready.');
  if (!Number.isInteger(width) || width < 320 || width > 7680 || width % 16) throw new Error('Width must be a multiple of 16 between 320 and 7680.');
  const canvas = capture(width, width * 9 / 16);
  if (Object.values(annotations).some(Boolean) || guides.length) await drawSvg(canvas, renderToStaticMarkup(<svg xmlns="http://www.w3.org/2000/svg" width={width} height={width * 9 / 16} viewBox="0 0 1600 900"><CameraOverlayContent shot={shot} annotations={annotations} guides={guides} /></svg>));
  return png(canvas);
}
