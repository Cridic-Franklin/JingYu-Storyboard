...
export function shotSpatialData(shot: Shot) {
  const cameraObject = shot.objects.find(o => o.type === 'Camera'); const camera = cameraObject ? storyboardCamera(cameraObject) : null;
  const primary = shot.objects.find(o => o.id === shot.primarySubjectId) ?? shot.objects.find(o => o.type === 'Character');
  const objects = shot.objects.filter(o => o.type !== 'Camera').map(o => {
    ...
    const target = shot.objects.filter(other => other.id !== o.id && other.visible && other.type !== 'Camera').map(other => ({ object: other, alignment: front.dot(new Vector3(...other.position).sub(new Vector3(...o.position)).normalize()) })).filter(p => p.alignment > 0.8).sort((a, b) => b.alignment - a.alignment)[0]?.object;
